...
useEffect(() => {
  ...
  const fetchSizes = async () => {
    try {
      ...
      if (product.subcategory) {
        q = query(
          collection(db, "sizes"),
          where("subcategoryName", "==", product.subCategory)
        );
      } 
      // ✅ ELSE → fallback to category
      else {
        q = query(
          collection(db, "sizes"),
      ...
      const sizeData = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  if (customerName || designation) {
    ...
    setMainImage(product.images[1] || product.images[0]);
  } else {
    // 🔥 Back to original
...
const selectedSizeData = sizes.find(s => s.id === selectedSize);
 ...
 useEffect(() => {
  ...
  const scrollRight = () => {
    ...
    const total=Number(product?.price) + Number(selectedSizeData?.price || 0)
  ...
  {isCustomizing && (
    <div style={{ position: "relative" }}>
      ...
      <div
        style={{
          position: "absolute",
          top: product?.textPosition?.top || "50%",
          left: product?.textPosition?.left || "50%",
...
<div className="d-flex flex-wrap gap-2 mb-3">
  {sizes.length > 0 ? (
    sizes.map((s) => (
      <Button
        key={s.id}
        variant={selectedSize === s.id ? "dark" : "outline-dark"}
        onClick={() => setSelectedSize(s.id)}
       className={selectedSize === s.id ? "text-white" : ""}
      >
      
        <small   className={selectedSize === s.id ? "text-white" : ""}>
    ...
    {[1, 2, 3, 4, 5].map((star) => {
      ...
      return (
        <FaStar
          ...
          style={{
            color: isActive ? "#ffc107" : "#ddd", // ✅ GOLD vs GRAY
            transition: "0.2s",
